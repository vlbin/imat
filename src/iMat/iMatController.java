...
public class iMatController implements Initializable {
    ...
    @FXML private Button navbarLogin;
    @FXML private Text errorLoginText;
    ...
    @FXML private Order activeOrder;

    @FXML private FlowPane welcomePane;
    ...
    @FXML private Text categoryHeading;
    ...
    @FXML private AnchorPane myPagesRoot;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        for (Product p : dataHandler.getProducts()) {
            ...
            switch (i) {
                case 1:
                    product = new ProductItem(item, this, "3 FÖR 2");
                    break;
                case 2:
                    product = new ProductItem(item, this, "50% RABATT");
                    break;
                case 3:
                    product = new ProductItem(item, this, "20% RABATT");
                    break;
                case 4:
                    product = new ProductItem(item, this, "3 FÖR 29");
                    break;
                default:
                    product = new ProductItem(item, this, "no");
                    break;
            }
        ...
        customCategories.add(new CustomCategory("Frukt & Bär", "citrus_fruit", "exotic_fruit", "melons", "fruit", "berry"));
        customCategories.add(new CustomCategory("Fröer & Nötter", "nuts_and_seeds"));
        customCategories.add(new CustomCategory("Godis", "sweet"));
        customCategories.add(new CustomCategory("Grönsaker & Rotfrukter", "vegetable_fruit", "root_vegetable", "cabbage", "pod"));
    ...
    public void goToKassa(Cart cart) {
        closeCart(cart);
        if (sessionActive) goToDelivery();
        else goTo(returningOrNew);
    }
    ...
    public void completeSignUpThree() {
        ...
        if (empty(cardExpireMonthInput) || cardExpireMonthInput.getText().length() != 2 || !isValidMonth(cardExpireMonthInput.getText())) displayError(cardExpireMonthInput);
        if (empty(cardExpireYearInput) ||cardExpireYearInput.getText().length() != 2 || !isValidYear(cardExpireYearInput.getText())) displayError(cardExpireYearInput);
